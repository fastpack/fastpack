...
let rec map_statement scope handler ((loc, statement) : Statement.t) =
  let statement = match statement with
    ...
    | Statement.Expression ({ expression; directive = _directive } as n) ->
      Statement.Expression { n with expression = map_expression scope handler expression; }
        ...
        cases = map_list scope handler
            (fun scope handler (loc, { Statement.Switch.Case. test; consequent }) ->
               (loc, {
                   ...
                   test = map_if_some scope handler map_expression test;
    ...
    | Statement.Return { argument } ->
      Statement.Return {
        argument = map_if_some scope handler map_expression argument;
      }
    ...
    | Statement.Try { block; handler = _handler; finalizer } ->
      Statement.Try {
        ...
        handler = map_if_some scope handler (fun scope handler (loc, { Statement.Try.CatchClause. param; body }) ->
            (loc, {
        ...
        finalizer = map_if_some scope handler map_block finalizer
      }
    ...
    | Statement.For { init; test; update; body } ->
      Statement.For {
        init = map_if_some scope handler
        ...
        test = map_if_some scope handler map_expression test;
        update = map_if_some scope handler map_expression update;
        body = map_statement scope handler body;
      }
...
and map_class scope handler ({Class. body = (body_loc, { body }); superClass; _} as n) =
  (** TODO: handle `classDecorators` *)
  (** TODO: handle `implements` *)
  {
    n with
    superClass = map_if_some scope handler map_expression superClass;
    body = (body_loc, {
        Class.Body.
        body = map_list scope handler (fun scope handler item ->
            ...
            | Class.Body.Property (loc, ({ key; value; _ } as n)) ->
              ...
              let value = map_if_some scope handler map_expression value in
...
and map_expression scope handler ((loc, expression) : Expression.t) =
  let expression = match expression with
      ...
      Expression.Array { 
        elements = map_list scope handler
            (fun scope handler -> function
               | None -> None
               | Some element -> Some (map_expression_or_spread scope handler element))
            elements
      }
    ...
    | Expression.Yield ({ argument; _ } as n) ->
      let argument = map_if_some scope handler map_expression argument in
...
and map_function scope handler (loc, ({ Function. params = (params, rest); body; _ } as n)) =
  let params =
    map_list scope handler map_pattern params
  in
  let rest =
    map_if_some scope handler
      (fun scope handler (loc, { Function.RestElement. argument })
        -> (loc, { Function.RestElement. argument = map_pattern scope handler argument}))
      rest
  in
